"""Rank-truncated linear algebra for hierarchical spike-and-slab GLMs.

The design matrix X is n x (p+1) with p potentially in the tens of thousands, so
forming or inverting (p+1) x (p+1) matrices is infeasible. Following the Woodbury/
Kailath identity (as in Johnston et al. 2016, Sec 4.1, Eq. 8), we keep a rank-l SVD
surrogate of X and push every weighted normal-equations solve through an l x l
system instead.
"""
import numpy as np
